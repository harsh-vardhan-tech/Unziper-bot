...
def output_choice_buttons(job_id: str, channel: str = "") -> list:
    rows = [
        [Button.inline("📲 Send here (in bot)", data=f"out:here:{job_id}")],
        [Button.inline(
            f"📢 Send to channel" if not channel else f"📢 Send to {channel}",
            data=f"out:ch:{job_id}"
        )],
        [Button.inline("❌ Cancel — Don't Send", data=f"out:cancel:{job_id}")],
    ]
    return rows
...
async def send_files(
    ...
    sent  = 0
    skip  = []
    STOP_BTN = [[Button.inline("⏹ Stop Sending", data=f"stop:{flag_key}")]]
    ...
    for i, fp in enumerate(files, 1):
        ...
        try:
            ...
            err = str(e)[:100]
            skip.append(f"`{name}`: {err}")
            # Tell user which file failed but continue
            try:
                await status_msg.edit(
                    f"⚠️ *Failed to send* `{name}`\n`{err}`\n\nContinuing…",
                    parse_mode="markdown",
                    buttons=STOP_BTN,
                )
        ...
        # Small delay between files to avoid flood limits
...
async def cb_output_choice(event):
    ...
    if mode == "ch" and not ch:
        # Put job back and ask user to set channel first
        _pending_output[job_id] = pending
        try:
            await status_msg.edit(
                "📢 *No channel set yet!*\n\n"
                ...
                "3️⃣ Or use channel ID: `/setchannel -1001234567890`\n\n"
                "_After setting channel, tap the button again._",
                parse_mode="markdown",
                buttons=output_choice_buttons(job_id, ""),
            )
        except Exception:
            pass
        await event.answer("Set channel first with /setchannel", alert=True)
        return

    target = ch if mode == "ch" else chat_id
    target_label = f"📢 {ch}" if mode == "ch" else "📲 this chat"
